...
import { localImports } from "../../packages/guards/src/local-import-closure.mjs";
 ...
 *
 * Derived from `entry`'s own local-import closure and narrowed to `scripts/row-claim/` plus the entry
 ...
 *
 *     localImports("scripts/row-claim.mjs")  ->  0     (the pre-#1019 `stripComments` defect)
 *     ruleFiles(...)                         ->  ["scripts/row-claim.mjs"]
 *
...
export function staleRuleReason({ repoRoot, entry, run, files } = {}) {
  ...
  const spec = files ?? rulePathspec(entry ?? resolve(root, "scripts/row-claim.mjs"), root);
