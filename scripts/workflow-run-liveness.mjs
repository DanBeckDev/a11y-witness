...
function main() {
  ...
  refuseUnknownFlags(KNOWN_FLAGS, { entry: import.meta.url, command: "node scripts/workflow-run-liveness.mjs" });
  const sha = flagValue(process.argv, "sha") ?? process.env.GITHUB_SHA;
  if (!sha) {
    console.error("Usage: node scripts/workflow-run-liveness.mjs --sha=<commit>\n"
