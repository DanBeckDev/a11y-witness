 ...
 * @returns {string | null}
 */
export function refusalForDraft(draft, capRefusal, board) {
 ...
 */
export function writeRecord(draft, dir) {
  const file = path.join(dir, recordFilename(draft));
...
function main() {
  ...
  const written = writeRecord(draft, path.join(ROOT, ACHIEVEMENTS_DIR));
  process.stdout.write(`board:record wrote ${path.relative(ROOT, written)}\n`
