 ...
 */
export function mergeReadiness({ pr, required, runs, mainTipIso, behindBy, closes = [], prLabels = [],
  session = null }) {
...
function facts(number) {
  ...
  const pr = JSON.parse(gh(["pr", "view", String(number), "--repo", REPO,
    "--json", "number,state,baseRefName,headRefOid,labels"]));
  ...
  return { pr, required, runs, mainTipIso, behindBy, closes, prLabels };
}
