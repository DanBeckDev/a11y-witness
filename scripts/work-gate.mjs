 ...
 * @param {any[] | null | undefined} rollup
...
export function decide({ prs, readyRows }) {
  ...
  for (const pr of prs) {
    ...
    if (checksSettledGreen(pr.statusCheckRollup) !== true) continue;
  ...
  if (unclaimed.length > 0) {
    ...
    orders.push({
      ...
      prompt: `${unclaimed.length} Ready row(s) unclaimed: ${rows}. Claim the oldest with `
        + "`node scripts/row-claim.mjs claim <n> --session=<you> --branch=agent/<branch>` and build it.",
...
function main() {
  refuseUnknownFlags([], { entry: import.meta.url, command: "node scripts/work-gate.mjs" });
  const prs = readPrs();
