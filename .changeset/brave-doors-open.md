...
The first command on the README now works. It was `npx a11y-witness …`, which returns `E404` — nothing is
...
`promote-candidate-0d498ef6.md`, a `major` on `@a11y-witness/scorer` that `promote:model` wrote itself. So
...
`npm pack --dry-run --json` puts `README.md` among the 55 files in `a11y-witness`'s tarball, so it ships;
