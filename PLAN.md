...
- [x] **Interaction model, part 2 (v1) — operate controls.** Added a keyboard-interaction pass: reset to top (Ctrl+Home), Tab through focusable controls capturing each focused announcement, stop when focus leaves the page into browser chrome, plus a disclosure-activation probe (Space) that records the state announced after activation. Wired into the judge. Demonstrated on the menu pages: the focused bare "button" on menus-bad drives a 4.1.2 finding (conf 0.98), the interaction-time view of the unnamed toggle.
- [ ] **Harden part 2.** The Tab-through is reliable on multi-control pages but flaky on minimal ones (sometimes captures only the document node); the state-change probe is implemented but not yet cleanly validated on a real JS disclosure (static fixtures have no toggle). Also add form-fill + submit to capture error/status announcements (3.3.1, 4.1.3).
