...
- [ ] **Extend part 2 (blocked on sparse-page focus).** On real multi-control pages the Tab-through is reliable, but on very sparse pages (a single button) Tab intermittently will not advance past the document node — which blocks a clean bad-disclosure demo and form-fill/submit (error/status, 3.3.1 / 4.1.3) on minimal fixtures. The disclosure-state probe itself is validated (a working disclosure captured "collapsed" -> "expanded"). The robust fix is to traverse interactive controls via NVDA quick-nav (`moveToNextButton` / `moveToNextFormField`, which DO work on sparse pages, as the structural passes prove) plus focus-mode handling, rather than raw Tab. `disclosure-bad.html` is authored and ready for when this lands. Added meanwhile: skip the document node, and retry once if the Tab-through comes back empty.
