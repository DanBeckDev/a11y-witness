...
- [x] **Interaction model, part 2 — operate controls.** Keyboard-interaction pass: reset to top (Ctrl+Home), Tab through focusable controls capturing each focused announcement, stop when focus leaves into browser chrome, plus a disclosure-activation probe (Space) recording the announced state change. Wired into the judge.
- [x] **Fixed the flakiness via the Guidepup docs.** Cross-referencing the official API surfaced `windowsActivate`, which explicitly focuses the Edge window instead of hoping the launch took focus — the actual cause of empty/partial captures. Now reliable: `disclosure-good` captures the proper "collapsed" -> "expanded" state change, and `menus-bad`'s focused bare "button" drives a 4.1.2 finding (conf 0.98).
- [x] **Refactored the interaction traversal to NVDA quick-nav** (`moveToNext`/`PreviousButton` + activation) instead of raw Tab, which stalled on sparse pages and escaped into the browser chrome. Validation pending (see outage below).
- [x] **Debug mode / structured diagnostics (all levels).** Every capture phase records a diagnostic (`browserLaunched`, `windowsActivate`, `nvdaStart`, `afterStart`, `readThrough` with stopReason + firstStepError, `structural`, `interaction` sweepLog, `done`) instead of a silent catch; surfaced via `server.log` and the CLI `--debug`, with a 0-announcement WARNING. `src/capture/nvda/capture-core.mjs`.
- [ ] **Recover the worker, then validate + finish part 2.** Diagnostics pinpointed an outage: `nvdaStart` ok, but `afterStart.lastSpoken` is empty and every read returns empty with no error — NVDA is running but producing no captured speech (the speech-capture channel died after the session's many kill/restart cycles, likely worsened by manually killing nvda.exe and a possible system-vs-portable NVDA conflict). Fix: reboot the Windows VM (re-login to the Proxmox console afterward, or configure autologon for unattended recovery; stop manually killing nvda.exe — let Guidepup manage it). Then smoke-test (W3C before), and validate the quick-nav interaction + finish the bad-disclosure demo and form-fill/submit (3.3.1 / 4.1.3).
