...
- [x] **Resolved the conformant-page false positive by research, not punting.** The recurring WAI finding was verified against W3C's own source markup (the "Change Text Size or Colors" control is a correct `<a>` with descriptive text) and confirmed a stable false positive, root-caused to the role-less skip-link/controls nav announced at the top of the page. Added a judge guard (descriptive text IS a name; reserve 4.1.2 for role-only-no-name). WAI now scores 0 false positives with no recall regression. The only remaining FPs are the demo's own wrapper chrome on the "after" fixture.
- [ ] **Expert calibration.** Strip the demo chrome from the "after" fixture (or rely on WAI as the clean conformant benchmark); calibrate severity/confidence and "task completable" against an expert; pre-register "trustworthy enough".
