...
- [x] **W3C tutorial baseline.** 8 paired good/bad pages authored fresh from the W3C WAI tutorials (images, forms, page structure, tables) with W3C-derived ground truth: `src/eval/pages/tutorials/`. Good pages score 0 findings; bad pages are caught in every topic. It surfaced (and then fixed, via a generalising hint) a real recall gap on missing table-header association. Authoritative, contamination-resistant, held-out.
- [ ] Grow the eval set further (menus/carousels tutorial topics, MDPI LLM-auditing dataset, public-sector accessibility statements, ACT Rules cases).
