...
- [ ] **Conformant pages still show a ~1 flaky minor false positive each** (criterion varies run to run, low confidence): the LLM noise floor. Per-criterion guards are whack-a-mole; the right lever is consensus mode (`JUDGE_CONSENSUS`), which keeps only recurring findings. Decide whether to make consensus the reporting default, and calibrate severity/confidence with an expert.
