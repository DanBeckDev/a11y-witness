...
- [ ] **Phase 1b — Harden announcement-text capture across NVDA versions.** The exact phrase announced after `act()` is NVDA-version-sensitive: CI's NVDA (setup-action 0.20.0) re-announces the document instead of the role=alert / state change, where the VM's NVDA captures it verbatim. So the good/bad error-text distinction is currently gated only by `npm run eval` on the committed fixtures, not by the CI capture gate. Harden by pinning a known-good NVDA version and/or re-reading the field's `aria-invalid` state after submit (a 3.3.1 signal that doesn't depend on catching a transient live-region announcement).
