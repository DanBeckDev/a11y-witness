...
| `gate:consumer-visible` | **can any changed file reach somebody who INSTALLS a package — asked of npm, not inferred from the path.** The changeset job used to refuse any file under a published package's `src`, which is a question about where a file LIVES; the question it means is whether a consumer can observe it. Measured on three PRs blocked in one night over `.test.ts` files: `npm pack --dry-run --json` on `worker-fleet` returns 153 files and **zero** tests. The false positive is not the cost — the prescribed escape is `changeset add --empty`, so a gate that cries wolf trains people to write the very silence it exists to prevent. It asks `npm pack` (the authority `gate:isolation` already trusts) rather than pattern-excluding `*.test.ts`, because three published packages legitimately ship sources. **The subtlety that would be catastrophic to get wrong:** `cli`, `judge` and `evidence` ship `dist` alone, so `judge/src/rules.ts` is in no tarball and is the most consumer-visible change this repo can make — a changed file counts when IT or its build output at the mirrored `dist/` path is packed. Fails toward refusing: a package npm cannot list is treated as fully visible and says so |
...
npm run capture:check -- --worker=http://REDACTED-INTERNAL-ADDRESS:8765    # the capture layer, ~2 min
```
...
- **`packages/nvda-worker/src/capture-core.mjs` only runs against NVDA on the Windows VM** — it has no local test.
  ...
  ```bash
  npm run capture:check -- --worker=http://REDACTED-INTERNAL-ADDRESS:8765   # ~2 min from the Mac
  ```
