...
| | |
|---|---|
| **rules** (`rule-ownership.json` → `decidedBy: "rules"`) | the only layer that MAY assert — but owning a subtype and conformance-mapping it are two independent facts, and **only 4 of the 16 rules-owned subtypes actually assert** — the other twelve map as `secondary` and report `cantTell`, deliberately, because they INFER the failure where the four READ it directly. The four are `1.1.1:missing-alt`, `1.1.1:filename-alt`, `4.1.2:unnamed-control` and `4.1.2:state-change-silent`; `asserting-subtypes.test.ts` pins both numbers and that membership against the artefacts, because this sentence read "4 of the 11 ... the other seven" while the real answer was 14 and ten — and it moved again on 2026-09-06 to 16 and twelve, when `1.4.2:autoplay-uncontrollable` and `2.4.7:focus-removed-on-receipt` were declared, both `modelHead: false` so no head is fitted. The test caught the staleness both times, which is the point of pinning a prose count to an artefact. Every mapping is declared in `act-rules.ts` in ACT format and pinned by `act-rules.test.ts`; `rule-ownership.json` does not carry it. Exact on every criterion it owns, 0 false positives across 1,183 conformant records. |
...
- **It is a node script and not one Ansible playbook, and that was MEASURED.** The two halves are in
  ...
  `a11y-pve` key, which the control plane does not have — verified 2026-08-25, `REDACTED-INTERNAL-ADDRESS:22` is open
...
- **Every `.mjs` CLI here ignored an unrecognised flag**, because they all parse argv by looking for what
  ...
  **ALL 53 are guarded as of 2026-09-06**, and `cli-flags.test.ts` DISCOVERS every argv-reading
