...
**Most recent rehearsal:** 2026-09-14, runs [34799110658](https://github.com/DanBeckDev/a11ign-v1-rehearsal/actions/runs/34799110658) and [34802952263](https://github.com/DanBeckDev/a11ign-v1-rehearsal/actions/runs/34802952263), against `507654d67` — main with #1510 and #1512, the two stage-11 fixes; the Action resolved to that sha exactly (run 1's and run 3's `Set up job` lines). Two completed runs on the documents' recommended page and task, jobs 6 m 26 s and 6 m 46 s; 141 announcements each, `jq -c .transcript a11ign-result.json | sha256sum` = `6c040860…` for both and for both rehearsal-4 runs, so the release candidate behaves as the rehearsed one did; 0 lived-experience findings, 3 axe violations inside the YouTube embed. A THIRD run, [34799670660](https://github.com/DanBeckDev/a11ign-v1-rehearsal/actions/runs/34799670660), the same workflow plus one echo step, was lost by the hosted runner after 50 m ("The hosted runner lost communication with the server", GitHub's annotation) with no log and no artifact; the run dispatched after it completed normally, so the loss is recorded as a finding (#915, the rehearsal-5 account) and tracked by its own rows, not treated as the candidate's behaviour. Account: #915, comment 5658601005; ruling in the comment after it. Eleven documentation differences are rows, fixed before the `latest` dist-tag; the first publish is `next`.
<!-- REHEARSAL:COMMIT 507654d67a5f31908674057cdf45160ce9c42e31 -->
