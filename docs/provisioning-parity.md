...
| Durable browser capture profile | creates `%LOCALAPPDATA%\a11ign\{edge,chrome}-profile`, honours `A11Y_BROWSER_PROFILE`/`A11Y_EDGE_PROFILE` overrides | creates the identical two directories, no override support | **SAME path**, script-only override convenience |
