...
**Nine boxes serving; one out.** In the inventory's own words, `a11y-worker-10` is **WITHDRAWN 2026-09-07,
awaiting a console visit** — it answers neither `/health`, nor SSH, nor ICMP, and `fleet:wake`'s magic
packet did not bring it back, which the Ansible README records as the one thing nothing can automate.

**A DATE DISCREPANCY, NOT YET RESOLVED.** `ceo`'s ruling says *"withdrawn since 6 September"*. The tree
says **2026-09-07**, consistently, in three places: `packages/control/ansible/inventory.example.yml`,
`docs/operational-lessons.md`'s `a11y-worker-10, WITHDRAWN 2026-09-07`, and CLAUDE.md's anchor to it.
**Use the tree's date**, since the ruling itself says to use the inventory's words. Raised with
`orchestrator` and `ceo`; if the box went down on the 6th and the tree records when somebody wrote it up,
that is a fix to the tree, not to one summary.

**Prefer words to arithmetic.** "Nine of ten" is only right if the reader knows `a11y-worker-1` is retired
and its number never reused. Say nine are serving and one is out awaiting a console visit.

**NO CAPACITY CLAIM beyond a number a `fleet:status` run produced today, with its time.** `fleet:*` is
inside this session's resource ban, so that figure must come from `orchestrator`; requested 9 September.
**If it has not arrived by 07:25, the line carries no capacity figure at all** — the ruling caps what may
be claimed, it does not require a claim to exist.
