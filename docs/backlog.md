...
| `capture-core.mjs` | 4,969 | **3,020 comment, 201 blank, 1,748 code** | **DONE, and superseded by the three-way split below: `capture-core.mjs` 334, `capture-setup.mjs` 1,575, `capture-probes.mjs` 3,129 (checked `wc -l` 2026-09-06). This row's own "4,856" was the state ONE post-mortem-move ago; the recapture-validated split further down this page (`capture-core.mjs 4,885 -> 362`) then ran, and `capture-core.mjs` has since drifted 362 -> 334 from unrelated later edits — the split, not the exact count, is what to trust. `capture-probes.mjs` moved 3,082 -> 3,129 the same day, moving the census calls in `navigateByStructure` before `probeRouteChange` (known-gaps.md §40).** |
