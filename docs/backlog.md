...
| `capture-core.mjs` | 4,969 | **3,020 comment, 201 blank, 1,748 code** | **DONE, and superseded by the three-way split below: `capture-core.mjs` 334, `capture-setup.mjs` 1,575, `capture-probes.mjs` 3,144 (checked `wc -l` 2026-09-06). This row's own "4,856" was the state ONE post-mortem-move ago; the recapture-validated split further down this page (`capture-core.mjs 4,885 -> 362`) then ran, and `capture-core.mjs` has since drifted 362 -> 334 from unrelated later edits, and `capture-probes.mjs` 3,082 -> 3,132 -> 3,144 across three changes landing on different days: the census moving before `probeRouteChange` and the 2.4.7 fix raising `FOCUS_EVENT_LOG_DIAGNOSTIC_LIMIT` (both known-gaps.md §40, merged together), then §43's `startedFrom`/`focusReset` fields added to `probeFocusReveal` (queued, not yet mergeable — see known-gaps.md §43). **Neither branch's own row was right after the first merge** — each counted its own change and not the other's, and `backlog-file-facts.test.ts` caught the difference at the merge, which is exactly what a pinned number is for — the split, not the exact count, is what to trust.** |
