...
|---|---|---|
| **Eight container roles the GRAMMAR parses and the WORKER does not strip** — `frame`, `grouping`, `group`, `dialog`, `menu`, `list`, `table`, `blockquote`. Found 2026-09-05 while pinning the four copies of "what a container prefix looks like" after Edge 152 renamed one. **Not automatically eight defects**, and applying the widening blind would be the wrong move: the worker's pattern feeds `dedupeKey`, and stripping `"list, "` from a key could collapse two genuinely different announcements into one — losing evidence rather than cleaning it. | Run the check the last change to that regex ran, and which its comment quotes: *"over all 24,774 sweep announcements the repeated strip changes 146 keys and reduces NONE to empty — the over-strip signature this would otherwise risk"*. Needs the corpus. `container-prefix-parity.test.ts` holds the ledger in both directions, so a NEW container role cannot join the grammar without someone deciding what the worker does with it — which is how `section` would have slipped through. | [`container-prefix-parity.test.ts`](../packages/nvda-worker/src/container-prefix-parity.test.ts) |
| **115 corpus records are labelled `3.3.2` for a page that SATISFIES 3.3.2** — found 2026-09-05 by re-reading the claim-bearing criteria against the audit's own tells. W3C is explicit that 3.3.2 does not require labels or instructions to be marked up or ASSOCIATED with their controls — that is 1.3.1 — and that a field can pass 3.3.2 while failing 1.3.1. `form-unlabelled.bad` is `<span>Recipient name</span><input>`: text presented to the user identifying the control, which is WCAG's own definition of a label. So the page fails **1.3.1 and 4.1.2** and satisfies the criterion it is filed under. **The RULE is fixed** — its 3.3.2 mapping is now `secondary`, so nothing asserts it — but the LABELS are not, and a label that names a criterion the page meets does not measure the model, it measures the label. | Re-declare the family's primary criterion (1.3.1 or 4.1.2, with 3.3.2 dropped or kept as a referral) and retrain. **Deliberately not done inside the mapping fix**: it moves 115 records and the ground truth every 3.3.2 number is computed from, which is a decision with its own evidence, not a tidy-up — the `postSubmitNames` rule. Note the mapping fix already stops the wrong ASSERTION reaching a user; this row is about the training data. | [audit](./wcag-criterion-audit.md) |
