...
| **Two NVDA settings that change WHAT IT SAYS are not pinned, so drift is invisible** — **STILL OPEN. WRITTEN 2026-09-06, deliberately NOT LANDED, and the stated PRECONDITION was wrong.** `reportNotSupportedLanguage` (`[speech]`, default `"speech"`) makes NVDA announce a language switch the synthesiser cannot voice, or beep, or say nothing, depending on its value. This row assumed `autoLanguageSwitching` (`[speech]`, default `true`) is the precondition for `reportLanguage` firing AT ALL — **researched from NVDA's own `source/speech/languageHandling.py` rather than re-asserted, and it does not hold**: `shouldMakeLangChangeCommand()`, the gate for inserting a language-change marker into the speech sequence, is `autoLanguageSwitching OR reportLanguage`, so `reportLanguage` alone still speaks the language. What `autoLanguageSwitching` actually preconditions is `reportNotSupportedLanguage` (`shouldReportNotSupported()` is `autoLanguageSwitching AND reportNotSupportedLanguage != "off"` — THIS is the real `[documentFormatting]`-shaped pair), and it separately changes `reportLanguage`'s OWN announcement: `getLangToReport()` reports a root code ("es") when it is on, the full code ("es_ES") when it is off — a different string for the identical passage. The `language-marked-silent-*` cases this row cited as the mechanism that would now catch a drift **no longer exist** — withdrawn 2026-09-05 as REFUTED (see `case-matrix.mjs`'s note where they stood) — so that specific consequence is moot; the settings remain worth pinning for the two effects above, confirmed from source rather than the withdrawn cases. | Both added to `CAPTURE_SETTINGS` with corrected `why` fields, and `captureSettingsDigest` moved to `nvda-logging.mjs` (was private to `server.mjs`, which imports guidepup and cannot be reached from a portable test) so the "digest moves" property has a real, mutation-checked test. **Still deliberately NOT merged**: `screenReaderSettings` is a capture cache key, and this must ride the next key change rather than spend a ~4.5 h recapture alone. | [known-gaps §36](./known-gaps.md), [`nvda-logging.mjs`](../packages/nvda-worker/src/nvda-logging.mjs) |
