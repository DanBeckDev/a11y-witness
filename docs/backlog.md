...
| **Live validation while typing is unobservable, and it is NVDA rather than the corpus** — §17 read this as a missing case. It is not: `validation-live-silent` was BUILT and WITHDRAWN, because §18 measures typing plus a polite region at **0 of N**. Six character echoes leave NVDA no idle moment, so the region is never announced — the same mechanism that gives a checkbox 2 of 6. A new case would be BLIND. | Nothing, until the live-region behaviour changes. It is recorded as a capability bound rather than a task. `speakTypedCharacters` is not the blocker and turning it on would not help. | [not-working §18](./not-working.md), [known-gaps §31](./known-gaps.md) |
