...
| ~~**EVERY criterion we make a claim about is checked against its official text**~~ — **DONE 2026-09-04: all 17 audited — 9 clean and 8 with findings.** The split was 11 `assessed` and 7 `partial` when the audit ran; it is **10 `assessed` and 8 `partial`** now, because 1.4.13 was corrected to `partial` on 2026-09-06 (its own note already argued for it: one of three bullets reached, reported as a referral). The audit's scope is unchanged — the same 18 criteria carry a claim; one of them moved between the two kinds. Every rule that can ASSERT has been read against its criterion. The findings are separate rows; the two that matter are the asserting ones. What remains of the audit is the 33 `out-of-scope` REASONS, which are claims too but of the harmless kind — a misread there produces a finding we never make, not one we make wrongly. | **DONE** — the record is [`docs/wcag-criterion-audit.md`](./wcag-criterion-audit.md), the repeatable procedure is the [`wcag-criterion-check` skill](../.claude/skills/wcag-criterion-check/SKILL.md). | [audit](./wcag-criterion-audit.md) |
...
## OPEN — five rows I told the CEO were "on the backlog" and did not write, 2026-09-06
...
| **No JS/TS test has ever examined a Python lab-job's exit codes for the partial-corpus question** — found while building the `gate-partial-corpus-contract.test.ts` above. `exit-code-contract.test.ts`'s own discovery filters to `.mjs`/`.ts` by design (`gateVerdict`/`fleetVerdict` are JS-only concepts), so `train-screenreader-model.py`, `diagnose-false-positives.py`, `audit-scorer-shortcuts.py`, `evaluate-screenreader-acceptance.py`, `audit_grants.py`, `audit_container_exits.py`, `audit_applicability.py` and `explain_feature.py` — all eight dispatched by named `lab-job.yml` jobs — have never been asked "does examining fewer records than expected surface as INCONCLUSIVE, or can it read as a clean pass". A second, smaller gap closed by the same unit: `scripts/isolation-gate.mjs` (the `gate:isolation` npm script) lives at repo-root `scripts/`, outside `exit-code-contract.test.ts`'s three-package walk, and was invisible to it entirely — now classified (it enumerates its own full target list, so there is no external population to fall short of) | Nobody has asked the question, not "the answer would be no" — a genuinely separate investigation from tonight's JS-side one, and Python has no `verdict.mjs` equivalent to check for | Read each of the eight against the same question `gateVerdict`'s header asks, and either give Python gates a shared verdict helper of their own or document per-script why none is needed, the same way this unit did for JS |
...
| `evaluate-screenreader-acceptance.py` | Yes — one `--data` file instead of the two-or-more repeats stability needs. Already fails LOUD (`raise SystemExit(1)`) with the message explicitly separated from a real instability ("NOT MEASURED" vs "FAILED") — the exact historical bug CLAUDE.md records, already closed | **Refuted — already correct**, with one named nuance: the bare exit code (0/1, no 2) cannot itself distinguish "could not measure" from "genuinely regressed" — only the JSON/message can. Not fixed: nothing today reads the exit code without the message |
| `audit_applicability.py` | `sweep()` (the actual gate, decides exit 0/1) examines every record, no silent skip. `would_gating()` (a PRINT-ONLY "if this were gated" cost estimate, does not affect the exit code) silently skips unfeaturizable records with no count reported | **Named, not fixed.** Cosmetic: an inflated "SAFE" estimate in a report a human reads before deciding to add a gate, never a release-gate false pass |
| `scripts/isolation-gate.mjs` covered separately — see the row above this section (`agent/exit-code-contract`) | | |
