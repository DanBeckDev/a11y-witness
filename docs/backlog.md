...
| ~~**Losing the async acceptance loses the recovery path.**~~ **STALE ROW, ALREADY FIXED — verified and mutation-checked 2026-09-05.** `pollForResult` (`capture-client.mjs`) now catches a transient failure on the initial accept POST and calls `reconcileLostAcceptance`, which asks the SAME id whether it is running, done, or truly unknown before ever minting a fresh one — §14.3's own remedy. `capture-async.test.ts` gained "A LOST 202 IS RECONCILED BY THE SAME ID, not thrown away" and "a POST THAT NEVER REACHED THE WORKER is retried with a fresh id, confirmed via the SAME one first" — both pass on HEAD. Mutation-checked today by bypassing reconciliation (`throw error` before the `isTransient` check): both tests failed as `ECONNRESET`/`socket hang up`, confirming the guard is real, then reverted clean. | verified + fresh mutation check, `git status --short` clean after | done |
| ~~**Result recall is not an idempotency contract.**~~ **STALE ROW, ALREADY DECIDED — verified and mutation-checked 2026-09-05.** The decision recorded is DOCUMENT, not implement duplicate suppression: `capture-results.mjs`'s own header and route doc now state "404 IS BOUNDED RESULT RECALL, NOT PROOF THE CAPTURE NEVER RAN", naming all three causes (never arrived, evicted, worker restarted) instead of overclaiming one. `capture-results.test.ts` gained an eviction test ("an EVICTED capture reads 404 exactly like an id that never existed, though it definitely ran") and an explicit no-payload-check test ("begin() on an id with a RETAINED result discards it unconditionally"), both passing on HEAD. **This unit agrees with the decision on the merits**: every real call site mints a fresh `randomUUID()` per capture, so id reuse across different payloads is not a pattern any legitimate caller produces — fingerprinting would add real complexity against a misuse this store's actual callers do not commit. Mutation-checked today by disabling `evictOldestDone`: the bound test failed (`bounded, got 11`) and the eviction test failed (`200 !== 404`), confirming both are real, then reverted clean. | verified + fresh mutation check, `git status --short` clean after | done |
| ~~**The timeout ladder does not bound the whole operation.**~~ **STALE ROW, ALREADY FIXED — verified and mutation-checked 2026-09-05.** Client-side: `awaitCompletion`/`reconcileLostAcceptance` now clip every sleep/progress-read/result-read to `remaining(deadline)` rather than a fixed constant (`capture-async.test.ts`'s "a tiny timeoutMs is not padded..."). Worker-side: `CAPTURE_CLIENT_TIMEOUT_MS` raised 560,000 → 620,000 specifically because `runCapture` spends up to `DESKTOP_PREPARE_TIMEOUT_MS` (60 s) BEFORE the hard-timeout-wrapped capture attempt (520 s) even starts — true worst case 580 s, not 520 s — and `budget-ladder.test.ts` now asserts the client ceiling against `prepare + hard timeout`, not the hard timeout alone, discovering every capture client rather than trusting a list. Mutation-checked today by reverting the constant to 560,000: "the shared client ceiling covers desktop preparation PLUS the hard timeout" failed with the exact arithmetic (`580000` computed, `560000 !== > 580000`), then reverted clean. | verified + fresh mutation check, `git status --short` clean after | done |
