...
|---|---|---|
| **3.2.1 fires on a combo box announcing its own autocomplete instructions.** `design-system.service.gov.uk/components/checkboxes/`: `"Search Design system, combo box, focused, collapsed, has auto complete, editable, When autocomplete results are available use up and down arrows…"`. That is an `aria-describedby` pattern, not a CONTEXT CHANGE. It is `mapping: "secondary"` so it referred rather than accused, and it is in the baseline | A referral on a conformant page is not automatically a defect — but "we referred it because we could not tell" should be the rule's stated claim, not an accident of what the predicate happens to match | 3.2.1's predicate read against the criterion's own text (the `wcag-criterion-check` skill), and either narrowed or its limit written into `criterion-coverage.ts`'s note |
| **The four original fixtures have no good siblings.** `skip-link-broken`, `route-title-stale`, `keyboard-unreachable-action`, `focus-order-tabindex` are all `inaccessible`-only. 1.4.13's pair (added 2026-09-06) is the first with both halves, and `real-page-corpus.test.ts` now permits a conformant fixture only as a DERIVED sibling of a failing one | A bad-only fixture shows a rule firing and never shows it staying silent on a conformant page reached through the same evidence channel. That is half the property, and the half that catches a rule which fires on everything | Each of the four gains a `good.html` sibling, and the fixture role captures whole. Cheap — the pages are already generated |
...
| **`lab-job.yml` sets `exitMeanings` only for `evidence-check`** — found by worker-contracts mid-investigation, recorded here so it is not lost with the unit. `rules-gate`, `rules-real-pages`, `stability` and `gate-probe-order` all genuinely adopt `gateVerdict`/`fleetVerdict` and therefore all answer in the same 0/1/2/3 contract, and none of them declares it | A job that answers in exit codes and does not say what they mean is this repo's own rule broken — the catalogue documents the contract for one job and leaves four readers to infer it | `exitMeanings` on every job whose script uses the shared verdict, DERIVED rather than hand-listed, with a test that fails when a fifth adopter appears undeclared |
| **The `everything` chain's own gate ordering was never re-checked after `modelHead: false` landed.** Not a known defect — an unexamined assumption. That field emptied a criterion for the first time, and the empty case surfaced at SEVEN sites over one night, two of them only when a job actually ran | Six of the seven were found by running the thing, not by reading it. The seventh was on the shipping path | A pass over every consumer of `subtypes_by_criterion_for` and of a criterion report, asking what an empty subtype list does there. `test_criterion_with_no_head.py` and `test_headless_criterion_loads.py` are two of the answers; nobody has asked how many more there are |
