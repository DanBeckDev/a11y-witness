...
| **2.4.7 needs the focus EVENT, and 1.4.13's probe is BUILT** — both were moved out of `out-of-scope` by the 2026-09-05 audit. **1.4.13 IS NOT DONE — validated 2026-09-05 and the probe cannot fire.** All 18 cases (15 corpus + 3 held-out) came back BLIND from `check-signals` on `probeFocusReveal`'s first capture, and the mark says why: `{asked: true, revealed: false, why: "nothing appeared on focus"}` — so the probe RAN and measured nothing, which is the distinction the mark exists to make. **The baseline is taken after the event it is meant to measure.** The tab ring proves it: stop 2 is the trigger `Booking reference` and stop 3 is `Read the full guidance` — the link inside the `hidden` panel — so `probeFocusOrder`, which runs first, ALREADY revealed the panel. `probeFocusReveal`'s `before` census therefore contains the revealed content and its delta is zero by construction. The probe's own comment names the trap and draws the opposite conclusion — *"comparing against the untouched document would count everything the focus probe itself revealed"* — and counting exactly that IS the finding. The fix is sequencing, not the verdict function: the reveal baseline must be taken before any focus probe has run. `focusRevealVerdict` and its unit tests are unaffected. Original entry follows. ~~done pending validation~~: `probeFocusReveal` takes three censuses and two focus reads either side of Escape, `focusRevealVerdict` decides them in `capture-pure.mjs` where it is testable without NVDA, and 15 corpus cases plus 3 held-out pairs exist. **2.4.7 is not**, and the reason is precise: F55 strips focus ON RECEIPT, and the resulting tab-stop list is identical to a control that was never focusable — which is 2.1.1's signature, so the finding would be attributed to the wrong criterion. | **The mechanism is a focus EVENT LOG, injected over the CDP socket the census already opens**: `focusin`/`focusout` in capture order, so F55 reads as *in then immediately out, never in `focusOrder`*, while an unreachable control produces NO `in` at all. That distinction is the whole finding. **The risk is real and is why it is not built yet**: the census evaluates a READ-ONLY expression, and a listener PERSISTS on the page under measurement — a heavier intervention than anything the capture does today, and it would ship unvalidated while `probeFocusReveal` is itself still awaiting its first capture. Sequence it after that: one unvalidated worker change at a time. | [audit](./wcag-criterion-audit.md) |
...
| **The verdict pipeline is blocked at stage 12/13 by TWO real-page findings, and this row named only one** — corrected 2026-09-05 by reading the job's own log rather than this row. `rules-real-pages` reports `FAIL — 2 problem(s)`: `1.1.1` on `cqc.org.uk/search/all?query=hospital` (`graphicUnnamed=2`), **and `3.2.1` on `service-manual.nhs.uk/design-system/components/text-input`**, which appears in no document and whose baseline entry is `[]`, so it is NEW rather than accepted. Note the timing, because it rules one explanation out: the 3.2.1 downgrade to `secondary` landed at `c32a934` (04 Sep 11:19) and the run was at `d60f743` (12:06), so the rule was ALREADY downgraded when it fired — this is a referral the gate counts, not an assertion, and the gate is criterion-level by design. Unread, so which of the three causes it is remains open. Stages 1–11 PASSED including `acceptance`, so three of §11's four gates are green. **Investigated 2026-09-04 and the investigation found a different defect**: the capture recorded the NUMBER and nothing about the two nodes, so it could not separate the rule's three causes — and the live page cannot either, since these are sites their publishers edit. There was also no way to FETCH a real-page capture at all; `lab:fetch` only knew synthetic ones. Both fixed: `real-page-capture` is a fetchable artifact, and the census now records `graphicUnnamedDetail` with each unnamed graphic's nearest NAMED ancestor — which is exactly what 1.1.1's Controls/Input exception turns on. | **Deploy and re-run**: the detail is a worker change, so `fleet:deploy` then re-dispatch `migration-verdict`. **The page is `role: "calibration"`** — checked 2026-09-05, after the morning's `--role=training` run captured 39/39 cleanly and did not include it. The next capture answers the question directly. **Do not `--update` to unblock the run** — that is how a baseline absorbs a defect. | [audit](./wcag-criterion-audit.md) |
