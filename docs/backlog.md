...
| | what would tell you it is fixed | detail |
|---|---|---|
| **THE PUBLISHED ACTION INSTALLS NVDA ITSELF FROM AN UNVERSIONED INSTALLER, AND NOTHING RECORDS WHICH BUILD — found 2026-09-06, and it outranks the Python one.** `action.yml` runs `npx --yes @guidepup/setup setup` then `install nvda`. **`@guidepup/setup` is not a dependency of any package.json in this repo** — `npx --yes` fetches it fresh at whatever npm resolves, every run. `@guidepup/guidepup` IS pinned at 0.31.0 by `package-lock.json`, and the NVDA cache key is `hashFiles('package-lock.json')` — **so the key describes the CLIENT and not the screen reader it installs.** Which NVDA a consumer runs is therefore decided by when their cache was first warmed, and survives every later change that does not touch the lockfile. No NVDA version reaches the report. | Pin `@guidepup/setup`; put that version and the resolved NVDA build in the cache key; record the NVDA version in the report beside the scorer `runtime` block. The report half is already built (`score.py`'s `runtime`, merged 2026-09-06). | **Why this outranks the Python case: NVDA is the INSTRUMENT.** `screenReaderVersion` is a capture cache key and a `fleet-consistency` MUST_MATCH field in this repo precisely because the version is evidence — guidepup 0.29.2 → 0.31.0 measurably changed captured output, and Edge 152 made `form` announce as `section` across every corpus form at once. The Action pins the client that talks to NVDA, caches under a key describing that client, and lets the screen reader float. Two consumers scanning one page a month apart can get different announcements with no way to know. |
