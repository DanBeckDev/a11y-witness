...
|---|---|---|
| **5 of 25 corpus subtypes have no held-out acceptance coverage — was 8, and the CAUSE was not what the row first said.** Opened 2026-09-05 assuming nobody had written the cases. **They could not be written**: `pair()` in `acceptance-matrix.mjs` took `probeForms` and `probeTables` BY NAME and dropped every other probe flag, and the generator enumerated the same two — so seven of the eight subtypes could not ask for the probe they need. *A gate that cannot represent a case cannot fail on it.* The remedy already existed and had been applied to ONE of two pipelines: the corpus generator forwards `probe*` by prefix and its comment says why — *"enumerating them is how this exact defect happened three times in one feature"*. **Fourth instance, inside the feature whose comment records the first three.** Cost: 3.2.1 and 3.2.2 were among the seven and their mapping was downgraded the same day, so the gate was blind to the two heads whose behaviour had just moved. | **Both hops now forward by prefix**, mutation-checked, and three pairs are added (3.2.1, 3.2.2, 1.3.1). The remaining five — `2.1.1:control-unreachable-by-keyboard`, `2.1.2:focus-trapped`, `2.4.1:skip-link-inert`, `2.4.2:route-title-stale`, `2.4.3:focus-order-scrambled` — need pages with real navigation or a focus trap. **Ordinary work now, not blocked.** | [`acceptance-matrix.test.ts`](../packages/lab/src/training/acceptance-matrix.test.ts) |
