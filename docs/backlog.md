...
|---|---|---|
| **1.4.13 and 2.4.7 are REACHABLE and nothing reaches them** — opened by the 2026-09-05 audit, which moved both out of `out-of-scope`. **1.4.13**: the criterion covers *"pointer hover OR KEYBOARD FOCUS"*, and its Dismissable bullet asks for a way to dismiss content *without moving focus* — which is exactly what `dialogEscape` already records. **2.4.7**: F55, *"script removes focus when focus is received"*, is not a pixel question, and **3.2.1's rule has named F55 as an unclosed gap `focusOrder` could witness the whole time** — so ONE probe closes a hole in three criteria, and no entry mentioned any other. | A probe that diffs the structural census across a focus change, plus the focus EVENT rather than the resulting tab-stop list — without the event, focus stripped on receipt is indistinguishable from focus never reached, which is 2.1.1's signature. **Corpus case first**, per §17: a probe built now produces evidence nothing can validate. | [audit](./wcag-criterion-audit.md) |
