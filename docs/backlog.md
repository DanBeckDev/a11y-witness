...
| **One corpus pair was split by the INSTRUMENT, and "reproducible" does not fit the mechanism** — `icon-button-unnamed.good` records `pointerParkFailed` while its mate does not, so the halves were measured differently: 4 of 6,975 captures, 1 splitting a pair. Recapturing with `--no-cache` and re-running the audit reported the identical split, which I read as REPRODUCIBLE. **That reading is suspect.** `parkPointer` shells out to PowerShell to move the OS cursor — it never touches the page, so a PAGE-SPECIFIC failure has no mechanism, and its own header calls the observed failures "transient spawn failures". Two independent 0.1% events is also implausible. | Read the fresh capture's `pointerParkFailed` mark — it carries the PowerShell error text, `attempts` and `ms`, which says at once whether it timed out at 5 s or failed to spawn. That needs one fetch from the lab and the fleet is mid-recapture. **The likelier answer is that the audit is pairing on a name that also matches a `+also-` variant**, in which case the split is real but on a different file than the one it names. | [not-working §11](./not-working.md) |
