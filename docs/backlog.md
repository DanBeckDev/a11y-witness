...
| **1.1.1's CONTROLS/INPUT exception is stated but not enforced — AND IT IS NOW FIRING** (was: latent) — found by the criterion audit 2026-09-04. "If non-text content is a control or accepts user input, then it has a NAME that describes its purpose", so an `<img>` inside a named button satisfies 1.1.1 through the BUTTON's name. `a11y-witness:unnamed-graphic-count` counts nameless tree images and would accuse one. **It fired on 2026-09-04, hours after the audit predicted the class**: `rules-real-pages` refused the verdict pipeline at stage 12 of 13 with ONE new finding — `1.1.1 www.cqc.org.uk/search/all?query=hospital`, `graphicUnnamed=2`, on a page whose opening announcements include `menu button, sub Menu, Search`. That is the shape of an image inside a named control. **Not yet confirmed** — the rule's own output names the three possible causes (the tool is wrong; the page changed, since these are live sites; or the finding is right and the publisher's claim is not) and only the third takes `--update`. It REFERS rather than asserts (`secondary` → `cantTell`), so no page is accused. | Exclude a nameless image whose nearest ancestor in the tree is a NAMED control from `graphicUnnamed`. Cheap, and testable against the AX tree without a capture. Until then the exception is documented rather than handled, and the difference is one conformant page away from mattering. | [audit](./wcag-criterion-audit.md) |
