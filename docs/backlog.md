...
| **3.1.2 has a corpus now; it still has no RULE** — 29 cases captured on real NVDA 2026-09-03, `check-signals` **PASS: 1,623 discriminating, 0 blind, 0 contaminated, 0 uncaptured**. So the signal fires on the unmarked passage and stays silent on the marked one, measured rather than assumed. What is missing is the layer that turns that into a finding: no `rules.ts` rule reads it, and `rule-ownership.json` does not own the subtype. | A rule on `language-unmarked`, then the registries a new criterion touches. **`SCORED_CRITERIA` cannot take 3.1.2 until a retrain has produced its head** — a test pins that list to the SHIPPED model — so this lands rules-decided first, exactly as 3.3.3 did. | [known-gaps §29](./known-gaps.md) |
