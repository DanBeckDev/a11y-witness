...
| **PREDICTED 2026-09-06, BEFORE THE RUN: the real-page gate will report NEW 2.4.7 FINDINGS, and that is the 2.4.7 bug rather than a v19 signal.** `packages/lab/baselines/real-page-findings.json` contains criteria `1.1.1, 1.3.1, 2.4.3, 3.3.2, 4.1.2` and **zero mentions of 2.4.7** — it was built before the F55 detector had evidence. The real-page recapture now running is at protocol 15, so those captures WILL carry `focusEvents` for the first time. And the false-positive mechanism is a focus TRAP claiming focus for a dialog: `image-missing-alt-behind-consent.good` fires it three times, and `rules:real-pages`'s own output names cookie-consent dialogs on GOV.UK Design System, caselaw.nationalarchives and check-for-flooding. **A cookie banner is a focus trap.** | Re-run `rules:real-pages` AFTER the 2.4.7 fix, not before — the CEO's sequencing already says so: *"do not run rules:real-pages against a rule known to be wrong and then again. One run that means something."* | Recorded BEFORE the result so nobody reads a 2.4.7-caused failure as v19's revert condition 2 failing. Two independent things would otherwise arrive as one number. |
