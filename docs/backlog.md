...
| ~~**3.1.2 cannot be decided by any layer here**~~ — **CLOSED 2026-09-04, and the held-out set is what closed it.** The screen reader cannot accuse an unmarked passage (silence is equally what a correct monolingual page produces) and a rule cannot either (it needs the language of the TEXT, which is the DOM's territory). The last possibility was a trained head: it fitted the 29 corpus cases and then scored four HELD-OUT languages at 0.506–0.604 against a 0.979 cut, having learned the corpus's four language NAMES. Same asymmetry as `3.3.2:placeholder-only` — the corpus knows what it wrote, the screen reader cannot hear it. Now `decidedBy: "unavailable"`, so "nobody decides this" stays visible. The corpus cases stay as ground truth for a future DOM join. | **DONE** | [known-gaps §36](./known-gaps.md) |
