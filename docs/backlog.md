...
|---|---|---|
| **8 of 25 corpus subtypes have NO held-out acceptance coverage** — found 2026-09-05 by comparing the two lists for the first time; `CLAUDE.md` names them as hand-maintained copies of one fact and calls that the cause of two identical pipeline failures. `training:evaluate-acceptance` reports `passed: true` having never examined those heads, and `RELEASE.md` quotes it as a release gate — *a gate that does not exercise what ships is not a gate*, for the fifth time here. Among the eight are **3.2.1 and 3.2.2, whose mapping was downgraded the same day**: heads whose behaviour changed and which the held-out set cannot see. | Acceptance pairs for the eight. The drift itself is now closed — `acceptance-matrix.test.ts` pins the ledger in both directions and mutation-checks each, so a NEW subtype cannot silently join the list; what remains is paying down the eight that were already there. | [`acceptance-matrix.test.ts`](../packages/lab/src/training/acceptance-matrix.test.ts) |
