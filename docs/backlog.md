...
| **REOPENED 2026-09-06 — the test still holds, the count did not.** §2's "ZERO free vetoes can now reach a report" is stale: the TEST is still right (a closable veto only matters if its subtype is NOT rules-decided), but computed today from the tracked baseline it answers **two**: `3.3.1:validation-error-silent` and `4.1.3:form-activation-silent` each carry one closable veto (`form_change_observed_absent`, new since the feature-cross retrain) and neither subtype has a `rule-ownership.json` entry, so both are model-decided. Not carelessness: nobody has run the deliberate-act baseline ritual against THIS reason for these two subtypes — the veto arrived with the schema bump and has not been read. | Read the exported corpus for `form_change_observed_absent` against `provenance.subtype` on these two subtypes' positives — the exact playbook §2 already used for `form_change_nonempty` — and classify: genuinely never observed (a probe-coverage gap), observed and page-empty (a real finding), or the classification is stale. | [not-working §2](./not-working.md) |
...
| | what would tell you it is fixed | detail |
|---|---|---|
| ~~**4.1.3 real-page grounding: the config is WIRED, the capture has not run**~~ — **THE CAPTURE HAS NOW RUN, `-e role=calibration`, 49/49 with zero failures, 2026-09-05.** Verified on the page itself rather than on the run's exit code: `w3.org/WAI/demos/bad/after/survey.html` came back with `interaction.formChanges` holding 2 entries and `postSubmitFields` holding 15, and the sweep log records `submit "submit, button" -> "Citylights Survey - Submission Failed Accessible Survey Page, document"`. So the configured form was actually filled and actually submitted on a real site, which is what the row was waiting for. The distinction the previous version of this row drew is exactly why it was checked that way: a `--role=training` run had already happened and touched none of this, because the survey page is one of the 50 `calibration` pages — *"a capture ran" and "this page was captured" are different claims.* **What remains is arithmetic, not capability:** `build-realism` should stop reporting `4.1.3: 0 of 37`, and the `everything` chain running now is what says so. Know the ceiling: ONE page, error path only, because the consent guard admits only origins whose publisher put the form there to be submitted, only the conformant half, and never a `success` state — so 1 of 37 is the honest number and not a shortfall. | **DONE**; the count is the chain's to report | [known-gaps §29](./known-gaps.md) |
