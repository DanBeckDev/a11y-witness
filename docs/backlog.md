...
|---|---|---|
| **TEN CONFORMANT RECORDS ARE FAILED BY A DETERMINISTIC RULE — 2026-09-06, and it blocks promotion of ANY weights.** `rules:gate` scored 1,398 conformant records and found 10 false positives: nine 2.1.2 (`keyboard-trap-modal-cycle.good`, `keyboard-trap-modal-escape.good` and their `+also-` variants) and one 1.1.1 (`image-missing-alt-behind-consent.good`). Every rule-owned subtype otherwise reads EXACT. **Zero false positives on conformant records is this project's headline claim**, so this is the one result it cannot ship with. The gate refused `promote` and that is the gate working. | **TWO-SIDED, and the second half is not optional:** `rules:gate` back to 0 false positives across all 1,398, AND the 2.1.2 positives it owns still caught, with the count printed. **A rule that goes quiet by going DEAF is a failure this repo has already recorded** — the 2.4.3 transcript rewrite went from 71% of conformant pages to 6% while catching 0 of the 4 corpus records it owned, and `rules:gate` is what refused it. | Likely but NOT established: the `anchorToTop` Escape hazard CLAUDE.md already records — Escape pressed in browse mode with focus on the body, which a dialog scoping its handler to itself never sees. To be diagnosed from one record's evidence, not adopted from that note. **Open question that changes what this IS:** the gate reports `focusOrder on 300 of 2796 record(s)`, so 2.1.2's cycling branch is being exercised — if it never was before, this is a LATENT DEFECT the recapture exposed rather than a regression. Unsettled; the local corpus copy predates the comparison. |
