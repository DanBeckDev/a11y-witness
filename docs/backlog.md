...
## Two things landed today that are NOT yet validated where it counts

Both are merged and both have a named check that has not run. Recorded here rather than left to memory,
because *"a rule that asks a human to remember something is a rule that gets broken"*.

| what | the check that has not run |
|---|---|
