...
|---|---|---|
| **`provisionRevision` hashes files as they sit on DISK, so it depends on `core.autocrlf` — a per-machine git setting nobody sets here** — found 2026-09-04 while decoding a fleet split. The stamp is a SHA256 over four files read with `Get-FileHash`, and Windows git checks them out CRLF by default. Measured: the same four blobs at HEAD hash to **`dbb7d33409a9341d` with CRLF and `1052b80ca42398c7` with LF**, and the fleet reports the CRLF value — so the number every box compares for equality is a fact about its git config as much as about its provisioning. There is no `.gitattributes` in this repo, so nothing pins it. A box cloned with `core.autocrlf=false` would stamp `1052b80c`, read INCONSISTENT for ever, and **`fleet:provision` could not converge it** — re-provisioning re-computes the same wrong hash. The stamp's own header says it exists so "two guests with different NVDA/Edge configuration cannot share a cache entry"; line endings are not configuration. | Pin it, then prove it: add the four paths to a `.gitattributes` (or normalise the bytes before hashing in `stamp-provision-revision.ps1`), and assert the LF and CRLF forms hash EQUAL in a test — the mutation check is to feed it CRLF and watch the old code disagree. **Note this cannot be seen from `fleet:status`**: it reports a split, never that one is unclosable, so the symptom is a box that re-provisions cleanly and stays inconsistent. | [`stamp-provision-revision.ps1`](../packages/worker-fleet/src/provisioning/stamp-provision-revision.ps1) |
