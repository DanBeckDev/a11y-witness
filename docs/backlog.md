...
## OPEN — "a check that answers correctly about the wrong population", **FOURTEEN** instances, named by `ceo`
...
| 14 | `rules:coverage`, named as the acceptance for a coverage claim about 2.4.6 and 4.1.3 | the 16 RULE-OWNED criteria, correctly and completely | that **neither 2.4.6 nor 4.1.3 is rule-owned** — `rule-ownership.json` has no subtype for either, so neither appears in that report at all, whatever is captured | Issue #34, and it is the SECOND acceptance criterion found today aimed at a population it cannot contain. **The prediction was made and then tested rather than assumed**: I said capturing the ten `label-vague` cases would not move the coverage table, ran `job=rules-coverage`, and it did not — the table lists 16 criteria and neither of the row's two is among them. That row had FOUR population errors at once: an `--only=` selector matching 0 of 1,657, two case families that do not exist under the names given, a criterion described as `partial` that is `assessed`, and this. All four would have reported something that was not an answer to the row's question |
...
was four for about six hours** — instances 5-14 were all found afterwards, none of them by the probes that
sweep used. Eight of the ten were `orchestrator`'s own, one of them committed while writing this row.
...
already fixed at its own site. Closing it would discard the one artefact that lets the next one be
recognised by shape rather than by cost. It leaves this page only if the shape stops recurring.

**A SUB-SHAPE HAS EMERGED AND IS WORTH NAMING SEPARATELY: an ISSUE'S ACCEPTANCE COMMAND.** Instances 13
and 14 are both a tracker row naming a check that cannot see the thing the row is about — #30's passed
(`PASS 84/84`, on 85 real pages holding none of the 18 dataset records it concerned) and #34's would have
failed for the wrong reason. **Neither is caught by asking whether the acceptance was RUN**, which is the
remedy issue #75 proposes. The missing property is that an acceptance must be able to FAIL FOR THE REASON
THE ROW IS ABOUT — this repo's own canary rule (*"a canary that cannot express the fault is worthless;
reproduce the fault with your test before trusting the test's verdict"*) pointed at a tracker row instead
of a capture. A row's acceptance is a canary, and nobody has been asked to show it can go red.
