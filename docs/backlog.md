...
- ~~**Audit every criterion against its official text**~~ — **DONE 2026-09-04**, all 17, while the
  verdict pipeline ran. 9 clean, 8 with findings, each finding its own row above. The residue is the 33
  `out-of-scope` REASONS, which can wait: a misread there costs a finding we never make.
...
| **3.2.1 and 3.2.2 ASSERT on a title change, and the criterion says a content change is not a context change** — found by the criterion audit 2026-09-04, same shape as the 3.3.3 row above. `change of context` is defined as a change of user agent, viewport, focus, or "content that changes the MEANING of the web page", and the criterion's own note adds: *"A change of content is not always a change of context. Changes in content, such as an expanding outline, dynamic menu, or a tab control do not necessarily change the context."* `a11y-witness:context-change-without-action` maps `conformance` on both criteria and fires on any title difference — so a page appending a result count, or an SPA putting the active filter in its title, is asserted as a failure while conforming. Two further gaps: **attribution is assumed** (a title moved by a timer is credited to the focus, with none of the `baselineQuiet`/`sameState` guards this repo uses elsewhere), and **F55 is missed** — "using script to remove focus when focus is received", where FOCUS is itself a change of context, so the failure happens with the title untouched. | Require the change to plausibly change MEANING rather than merely differ — or downgrade to `secondary`, which is the same decision the 3.3.3 row asks for and should be taken together. Add an attribution guard before asserting. `focusOrder` could witness F55 and does not. All three are now in the rule's `assumptions`. | [audit](./wcag-criterion-audit.md) |
| **3.3.3 ASSERTS a conformance failure and does not guard either of the criterion's two exceptions** — found by the criterion audit 2026-09-04, and the most serious thing it has turned up. The criterion is *"If an input error is automatically detected **and suggestions for correction are known**, then the suggestions are provided to the user, **unless it would jeopardize the security or purpose of the content**."* `a11y-witness:error-remedy-missing` maps `conformance` and fires whenever an announced error carries no instruction. So **"Incorrect password" is asserted as a 3.3.3 failure**, when withholding the reason is required behaviour; and "That username is taken" is too, when no correction exists to suggest. It cannot fire on a real page today — `probeForms` is off in the CLI and for real-page captures — but **the GitHub Action defaults it ON, against the consumer's own application**, which is precisely where a login form lives. | Two pieces, and the first is cheap. **(a) The security exception**: do not assert where the post-submit re-read announces a password or protected field — NVDA announces `type=password` distinctly, so this is readable from `postSubmitFields`. **(b) The "suggestions known" condition is not readable from an announcement at all**, which is an argument that the mapping should be `secondary` rather than `conformance`. That is a decision about what the product asserts, so it is stated here rather than taken. Both exceptions are now in the rule's `assumptions`. | [audit](./wcag-criterion-audit.md) |
...
| **1.1.1's CONTROLS/INPUT exception is stated but not enforced — AND IT IS NOW FIRING** (was: latent) — found by the criterion audit 2026-09-04. "If non-text content is a control or accepts user input, then it has a NAME that describes its purpose", so an `<img>` inside a named button satisfies 1.1.1 through the BUTTON's name. `a11y-witness:unnamed-graphic-count` counts nameless tree images and would accuse one. **It fired on 2026-09-04, hours after the audit predicted the class**: `rules-real-pages` refused the verdict pipeline at stage 12 of 13 with ONE new finding — `1.1.1 www.cqc.org.uk/search/all?query=hospital`, `graphicUnnamed=2`, on a page whose opening announcements include `menu button, sub Menu, Search`. **I called that the Controls/Input exception and INSPECTING THE PAGE REFUTED IT.** Every `<img>` there carries `alt=""` (decorative, ignored by Chromium, never counted), so the two counted can only be bare `<svg>` with no role and no label — and while some sit inside named links, several are inside no control at all. An exposed unlabelled `<svg>` outside a control is plausibly a REAL 1.1.1 finding: if decorative it should be `aria-hidden`. So this may be the rule's third cause, not its first. It REFERS rather than asserts (`secondary` → `cantTell`), so no page is accused. | Exclude a nameless image whose nearest ancestor in the tree is a NAMED control from `graphicUnnamed`. Cheap, and testable against the AX tree without a capture. Until then the exception is documented rather than handled, and the difference is one conformant page away from mattering. | [audit](./wcag-criterion-audit.md) |

| **4.1.3 covers ONE of the criterion's four status-message categories** — found by the criterion audit 2026-09-04. WCAG defines a status message as content change reporting "the success or results of an action, the waiting state of an application, the progress of a process, or the existence of errors". The single subtype `4.1.3:form-activation-silent` covers the first and overlaps the fourth with 3.3.1. WAITING STATE and PROGRESS are uncovered. | Corpus cases for a live region announcing a waiting or progress state — "Loading…", "3 of 10 complete". Reachable: such a region is as audible as any other, and the delta channel `formChanges[].after` already captures speech the page produces on its own. **Use that channel, never `postSubmitFields`** — the criterion requires presentation "without receiving focus", and a re-read cannot show it. | [audit](./wcag-criterion-audit.md) |

| **4.1.2's SETTABILITY clause is absent from our enumeration of it** — found by the criterion audit 2026-09-04. The criterion has three clauses: "the name and role can be programmatically determined; **states, properties, and values that can be set by the user can be programmatically set**; and notification of changes ... is available". We cover the first and the third. `criterion-coverage.ts` says "two of three failure modes", and its third is a role-less `<div onclick>` — which is a mode of the FIRST clause, not a third clause. So the entry reads as covering more of the criterion than it does. | State the clause in `criterion-coverage.ts` so the boundary is visible. Whether it can be TESTED is a separate question: it asks whether an AT can programmatically SET a value, which a capture would have to do rather than observe — `probeArrows` and `probeTyping` already write to controls, so it may be reachable. Decide that after stating it. | [audit](./wcag-criterion-audit.md) |

| **The verdict pipeline is blocked at stage 12 of 13 by one unresolved real-page finding** — `1.1.1` on `cqc.org.uk/search/all?query=hospital`, `graphicUnnamed=2`. Stages 1–11 PASSED, including `acceptance`, so three of §11's four gates are green and only this one stands between the run and §11's verdict plus 4.1.3's grounding. | Read the CAPTURE's evidence, not the live page — these are sites their publishers edit, so the page loaded today is not the page captured. `capture:explain` and the census `names` are the inputs. Then one of three: fix the rule (if the two are inside named controls), accept the page changed, or `--update` (only if the finding is right and the publisher's claim is not). **Do not `--update` to unblock the pipeline** — that is how a baseline absorbs a defect. | [audit](./wcag-criterion-audit.md) |
