...
| `NVDA is running but not speaking` | **normal, and expected roughly every 5th capture.** NVDA answers keystrokes but its speech channel has died. Measured lifespans over 30 back-to-back captures: 6, 5, 5, 9, 1 — so `MAX_CAPTURES_PER_NVDA = 25` never fires, and reuse is causal (with `reuseScreenReader:false`, 8 of 8 ran clean) | **nothing to do.** The worker retries once itself on a fresh NVDA before answering, so the caller never sees it. Do **not** reboot the guest for this. Watch `/health.vitals.recoveries` instead: if it climbs while captures still succeed, the guest is degrading and papering over it |
