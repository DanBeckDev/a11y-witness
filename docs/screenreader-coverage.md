...
|---|---|---|
| **Status messages / live regions**, NARROWED 2026-08-31 | This said we catch them "solely as a side effect of form submit". **Not so** — `probeKindFor` also activates a button the TASK names, and all 143 `4.1.3:form-activation-silent` cases use exactly that (`kind: taskButton`), so "notice how many results remain" is tested. The REAL gap is a live region triggered by something that is not a button: a link, a `<select>`, a checkbox. And widening it is bounded by policy, not by code — `probeForms` is off in the CLI because *pressing Book on a stranger's site is not a review* (`SECURITY.md`) | `activateAndCaptureDelta` already does the work; what needs deciding is WHICH controls it may reach, and that is a `SECURITY.md` question before it is a capture one |
...
| **iframes**, SWEEP BUILT 2026-08-31 | The sweep is written — `moveToNextFrame`/`moveToPreviousFrame` as a sixth `EXTRA_SWEEPS` entry, `anchorFirst` for the reason `lists` pays it: a frame CONTAINS what was swept before it, and quick-nav cannot find the container the caret stands inside. **Not yet active**: it needs a `CAPTURE_PROTOCOL_VERSION` bump to run, and that is deliberately bundled rather than spent on one sweep. **And it has no corpus case yet, deliberately** — a case needs a `badSignal` that discriminates named from unnamed, which needs to know what NVDA actually announces for each, which is a capture question. Guessing it is how a signal comes to fire on both variants | built; capture a frame, read the announcement, then write the case |
...
| **SPA route changes**, NARROWED 2026-09-01 | This said the route-change probe was undriven. It is not: `probeRouteChange` already activates the first link, captures the announcement delta through `activateAndCaptureDelta`, and records title and first-heading before and after — which is exactly "activate a link, wait, diff the log". `2.4.2:route-title-stale` reads it and is a shipped rule. **Two things are genuinely missing.** `routeChange.announced` is recorded and read by NOTHING. And this row asserts the failure is 4.1.3 without support — 4.1.3 is about status messages that do NOT take focus, and a navigation does; getting that mapping right is the work, and building a rule on the row's own say-so is how the heading-hierarchy entry went wrong | settle the criterion first, then a rule may read `routeChange.announced` |
