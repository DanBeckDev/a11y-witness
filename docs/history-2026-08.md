# Project history to 2026-08-09 — archived from PLAN.md

`PLAN.md` had grown to 1,299 lines, most of it a record of work already finished and defects already fixed.
It was bulldozed on 2026-08-09 so it could be a backlog again. This is what was in it.

**Kept rather than deleted, deliberately.** Much of the value here is not the checkboxes but the MEASURED
numbers and the wrong theories — recorded so nobody re-runs an experiment that already failed. Where a
finding is load-bearing for how the code works today it also lives in `CLAUDE.md`, an ADR, or the comment
at the relevant call site; this file is the audit trail behind those.

Read it when you want to know *why* something is the way it is, or before re-attempting something that
looks obviously worth trying. Do not read it as a to-do list — everything still outstanding moved to
`PLAN.md`'s release backlog.
