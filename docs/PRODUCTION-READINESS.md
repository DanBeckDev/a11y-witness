...
| 2 | Configure | **Implemented, fleet refresh pending** | Provisioning sets NVDA config, Edge policies, `ForegroundLockTimeout`, auto-logon and the scheduled tasks. The worker now resolves `provisionRevision` from its actual checkout. The current Mac UTM registry has no registered worker VMs, so the configured fleet still needs to be restored and re-provisioned before this is verified in the production-shaped environment. |
| 3 | Provision | **Done** | Local UTM VMs via `build-vm.sh` / `clone-worker.sh`; CI via `guidepup/setup-action`. |
| 4 | Deploy | **Implemented, verification pending** | `npm run worker:deploy` — pushes every hashed file, reboots (mandatory: `utmctl exec` cannot be trusted to restart the worker), then verifies `/health.code` over HTTP. The command is ready, but the current UTM registry contains no registered guests, so a deployment has not yet been verified against this checkout. Rollback is `git checkout <ref> && npm run worker:deploy`; git is the source of truth, so there is no bespoke backup to go stale. |
...
| 15 | Tests | **In progress — release gates implemented** | 182 unit tests pass, but all 1,061 existing pairs are stale under the current page/provenance guard and the model report is not release-eligible. The corpus gate, `capture-check`, `evidence:check`, acceptance evaluation, and judge eval still require their respective current capture/worker/auth prerequisites. |
