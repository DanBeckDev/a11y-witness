...
"did the evidence move?"). Also the name of `@a11ign/evidence`, the package
...
`@a11ign/scorer`, where **the weights are the API**: a retrain is a major
...
`@a11ign/judge`. The distinction from *scorer* is load-bearing: `scoreCapture`
...
`/health` and `/diagnostics` over HTTP. Shipped as `@a11ign/nvda-worker`,
...
imports guidepup. Shipped as `@a11ign/worker-fleet`. A *worker* is one guest;
...
`@a11ign/lab`. It is also the first consumer of every public API.
