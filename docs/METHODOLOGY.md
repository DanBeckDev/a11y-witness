...
| Human-agreement baseline | Partial | No direct expert labelling yet, but the ground truth is mostly not ours, and that is now **measured rather than asserted**: of 34 eval cases, **4 come from a third-party report on that exact page** (W3C BAD), **28 from a third party's documented technique** (W3C tutorials, Firth, the Cookbook), and **2 are authored by us** — the planted and contamination tests, where knowing the answer IS the test. Restricted to the 18 failure cases recall is computed over, **16 of 18 (89%)** have third-party ground truth. Pinned by `cases.test.ts`, which fails if the authored set grows or drops below 80%. A live expert-labelled sample is still wanted. |
...
| Reproducibility | Substantial (default backend) | **This row described a rented model and was stale from 2026-08-04**, when `JUDGE_BACKEND` began defaulting to `local`; every clause of the old text is false for what now ships. The trained scorer is 821 KB of safetensors **committed to the repo**, over a frozen MiniLM encoder pinned by git revision (`1110a243…`) *and* verified by SHA-256 at fetch. The forward pass is `.eval()` + `torch.no_grad()` with **no sampling anywhere** — no temperature, no seed to control — so the same capture yields the same score, and no prompt is involved at all. What is left: results can vary in the last bits across BLAS implementations and hardware, and the encoder is fetched once rather than vendored. The rented backends (`anthropic`, `openai`, `codex`) keep every caveat the old text described, and are **comparison only**. |
