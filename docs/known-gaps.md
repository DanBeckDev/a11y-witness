...
- [§21](#21-413-real-page-grounding-unblocked-2026-09-03-and-demonstrated-on-a-real-pair-still-open-and-this-is-corrected-from-an-earlier-version-of-this-audit) 4.1.3 real-page grounding — UNBLOCKED 2026-09-03, and demonstrated on a real pair — STILL OPEN, and this is CORRECTED from an earlier version of this audit
- [§28](#28-four-criteria-were-unreachable-on-a-page-we-do-not-own-working-2026-09-02) Four criteria were unreachable on a page we do not own — WORKING 2026-09-02
...
**`found` and the census describe two different moments of a live page, minutes apart, and one of the two instruments modifies the page it is measuring.** The census is READ first — `censusBeforeNavigating()` is the first thing in `navigateByStructure` (`capture-probes.mjs:583`), moved there by §40's GOV.UK incident — but it is MARKED after every sweep has finished (`capture-probes.mjs:201`), and `atMs` is stamped at push time (`capture-core.mjs:132`). **So `structureCensus.atMs` reports ~450 s on a capture whose census was taken at ~0 s**, and reading the capture at face value says the census is a post-sweep snapshot. It is a pre-load one. Meanwhile `heading` is walked at ~100 s, `formField` at ~285–402 s, and `formField`'s `onItem` **activates 64 controls as it walks** (`activationBudget.spentMs 121811`). A ratio built on that pair is a real number about two pages.

The cleanest evidence is `heading`, clean precisely because it has no `onItem`: `found` is **80 on all five captures**, while the census reports 83 at load in the morning and 69 in the afternoon. **The numerator holds still across a page-shape change that moved the denominator 17%.** A denominator that moves while its numerator does not is not that numerator's control. The morning and afternoon captures are a third confound on top of both — at load they are different pages (`formControl` 224 vs 125, `link` 372 vs 295, `graphic` 206 vs 128, `heading` 83 vs 69), so the 0.45s and the 2.1s were never two readings of one thing.
