...
**What this does NOT establish** was whether IKEA genuinely serves 265 form controls or the sweep walks more than is there. **#800 asked it and #836 answered it: neither.** Across five captures of that URL, every one `targetMatch: matched`, `sweep.found` against the RAW `formControl` census gives 0.45 0.45 2.27 2.17 2.12 — **the comparison changes sign on one page in one day**, while `heading` (0.96–1.16) and `landmark` (0.86) agree across the same five. A denominator whose comparison inverts is not measuring its numerator's population, and the instruments can agree, so the disagreement belongs to one bucket rather than to the census. The three counts are three definitions: `domCensus.formField` counts DOM elements, `structureCensus.formControl` counts AX nodes in `FORM_CONTROL_ROLES` (which excludes `link`, `menuitem`, `option` and `tab`), and `sweep.found` counts distinct announcements. The successor is a census-definition row, not the deadline row this section originally expected.
