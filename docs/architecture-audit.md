...
| provisioning | **MEASURED AND DECIDED** — see [`docs/provisioning-parity.md`](provisioning-parity.md). The role is more correct on every divergence with real consequences (Edge pin and enforcement, the blank-password guard's severity, the Wake-on-LAN-preserving NIC fallback); the script's real remaining audience was never "a fleet box with no Ansible" but the solo local-worker workflow in `docs/getting-started.md`. Parity is no longer the goal for the fleet; retiring `provision.yml` from fleet use costs one recapture (quantified, not executed this pass) | the fleet path: `roles/worker/`, decided. The solo path: `provision-nvda-worker.ps1`, unchanged |
| `provisionRevision`, a capture cache key | **STALE ABOVE, CORRECTED** — see [`docs/provisioning-parity.md`](provisioning-parity.md#provisionrevision--the-measurement-the-whole-compare-and-see-procedure-depends-on). Computed once (`stamp-provision-revision.ps1:106-121`) from **five** files, not four (`a11y_speech_viewer.ps1` was added 2026-09-05): the script, `run-server.cmd`, `apply-foreground-lock-timeout.ps1`, the role's `defaults/main.yml`, and that one module. So it is no longer true that it covers "not the role's modules" — one module IS hashed. What remains true and is now the open item: the other **five** `a11y.worker` modules and all **ten** task files are still unhashed, by an explicit design decision for task files (documented in the stamp's own header) that was never re-argued for modules, which reconcile real environment state the same way `a11y_speech_viewer.ps1` does | the file list is pinned by nothing; open work, not yet a backlog row — see below |
