...
- **A capture must refuse a page nothing is serving. DONE 2026-08-28**, proved on bare-metal
  ...
  **A guard for this already existed and structurally could not see it.** `BROWSER_ERROR_TITLE_RE` matches
  ...
  `http://REDACTED-INTERNAL-ADDRESS:3000/x` is titled `REDACTED-INTERNAL-ADDRESS` and matched nothing. A title is a proxy for the
  ...
  returns `focusOrder: ["REDACTED-INTERNAL-ADDRESS, document, read only"]` and reads as a valid capture of a document.
