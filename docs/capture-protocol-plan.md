...
| **"a silent connection is reaped by NAT" — MY OWN diagnosis** | **REFUTED TWICE OVER.** By TOPOLOGY: the control plane is `REDACTED-INTERNAL-ADDRESS/24`, every worker is `192.168.1.x/24`, and the route is `link#14` — direct, no gateway hop. **There is no NAT between these hosts at all**; it happens at `REDACTED-INTERNAL-ADDRESS` on the way out. And by MEASUREMENT: 0 of 23 lost with keepalive explicitly OFF — 8 sequential, then 15 across five boxes at once, the exact condition the 9/40 came from. At the old 22% rate P(0 in 23) = 0.4% |
