...
started as a11y-pipeline-gates on REDACTED-INTERNAL-ADDRESS. It now outlives this terminal.
...
...
Measured 2026-08-29 on CT 120: **no `node_modules`**, the fleet key present, and `REDACTED-INTERNAL-ADDRESS:22`
...
`fleet-playbook.mjs` SSHes to `REDACTED-INTERNAL-ADDRESS` and runs `ansible-playbook` THERE — its header says so
