...
| [schema-migration-history.md](./schema-migration-history.md) | **why each scorer schema migration was opened, and how it closed** (v16 → v19) — modelled on `capture-protocol-version-history.md` above for the identical reason: `schema-migration.json` is the live toggle and is DELETED the moment a migration closes, so citing it as "the record" cites a file guaranteed to be gone (#340) |
