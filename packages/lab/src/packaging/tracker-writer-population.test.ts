 ...
 */
const PROBE_ARGV = ["--version", "--body", "the box at REDACTED-INTERNAL-ADDRESS answered"];

test("#1053: the guard throws on a leaking body BEFORE the spawn, and the probe cannot write either way", () => {
  assert.equal(bodyFromArgv(PROBE_ARGV), "the box at REDACTED-INTERNAL-ADDRESS answered");
  ...
  assert.doesNotThrow(() => assertNoLeakInArgv("git", ["commit", "-m", "the box at REDACTED-INTERNAL-ADDRESS"]),
