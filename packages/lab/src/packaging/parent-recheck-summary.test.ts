...
test("#744 HYPOTHESIS, REPRODUCED: a worktree pinned at an OLD commit still sees origin/main AS IT IS NOW, "
  ...
  try {
    ...
    // BEFORE: the commit the #616 re-check will pin the parent worktree at -- standing in for the commit
    // immediately preceding a red push, whose own trunk-guard run was genuinely green at the time.
    commit("a.txt", "1\n", "BEFORE");
    git(primary, ["push", "-q", "-u", "origin", "main"]);
    const before = git(primary, ["rev-parse", "HEAD"]).trim();

    // THE RED PUSH (what #616's re-check is investigating) -- irrelevant to this reproduction beyond
    // existing, so origin/main has somewhere to be that is not `before`.
    commit("a.txt", "2\n", "the push under investigation");
    git(primary, ["push", "-q", "origin", "main"]);

    // TIME PASSES: main moves again, simulating anything landing between the original trunk-guard run and
    // the re-check running now -- a follow-up fix, an unrelated merge, another push entirely.
    commit("a.txt", "3\n", "main moved on, unrelated to the push under investigation");
    git(primary, ["push", "-q", "origin", "main"]);
    const currentOriginMain = git(primary, ["rev-parse", "origin/main"]).trim();

    // THE RE-CHECK ITSELF: pin a worktree at BEFORE, exactly as trunk-guard.yml's real step does.
