...
import {
  sourceClosure, discoverTestFiles, selectTests, broadReasons,
...
test("broadReasons: a root config file or a scripts/*.mjs file IS a reason -- ci-changed.mjs's own "
  + "coarser rule already handles these correctly and this file steps aside for it", () => {
  const files = ["package.json", "scripts/merge-guard.mjs", "packages/evidence/src/wcag.ts"];
  assert.deepEqual(broadReasons(files), ["package.json", "scripts/merge-guard.mjs"]);
});
...
test("selectTests: MUTATION TARGET -- a changed source file with ZERO reaching tests falls back to its "
  ...
  const result = selectTests(changed, () => new Set(), [], "/repo");
...
test("selectTests: a DIRECTLY changed test file selects itself even when no closure reaches it -- it is "
  ...
  const result = selectTests(["packages/pkg-a/src/self.test.ts"], () => new Set(), [], "/repo");
