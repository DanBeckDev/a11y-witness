...
export interface RuleInput {
   ...
   *
   * NOT YET POPULATED BY ANY CAPTURE, and DELIBERATELY NOT YET READ BY A RULE — issue #79 declared this
   * channel and the provisional corpus case (`input-purpose-*` in case-matrix.mjs) that exercises it, but
   * held the F107 rule itself (`addUnidentifiedInputPurpose`, ACT rule 73f2c2) out of that PR on purpose:
   * `RULE_CRITERIA` in `coverage.ts` carries a hard runtime throw for any criterion a rule reports that
   * is not listed there, so registering the rule makes it PUBLIC in `action.yml`/`RELEASE.md`'s
   * stranger-facing counts whether or not any capture can ever populate this field — the same "the rule
   * never fired" vs "the rule never had its evidence" distinction `docs/backlog.md` already records for
   * `1.3.1:no-headings`. #170 is a worker-side census reading this attribute (mirroring `mediaCensus`,
   * `packages/nvda-worker/src/browser-session.mjs`) — fleet-touching, which is not this unit's region to
   * run — and the F107 rule lands together WITH it, on one sha, so the public count moves only once real
   * evidence exists. Absent means NOT CHECKED, exactly as `media`'s own comment states.
   */
