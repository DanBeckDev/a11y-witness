...
test("every key oracleCounts returns is declared on RuleInput", () => {
  ...
  const readable: Required<Pick<RuleInput,
    "census" | "dom" | "probes" | "completeness" | "truncated" | "media" | "formInputs">> = {
      ...
      media: [],
      // `formInputs` joined with 1.3.5 (#869, issue #79), same shape as `media` just above -- named here
      // for the identical reason, even though no worker census populates it on any capture yet.
      formInputs: [],
    };
