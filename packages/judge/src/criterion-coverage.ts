...
export const CRITERION_COVERAGE: Record<string, CriterionCoverage> = {
  ...
  // the status said `assessed`, and a reader could not tell which of the two to trust.
  //
  ...
  // assessed, but a NAMED failure mode of it is not covered. Read note." This entry's note NAMES two of
  // the criterion's four categories -- waiting state and progress -- and says outright they are not
  // covered. That is `partial`'s definition applied to its own subject, not a paraphrase of it.
