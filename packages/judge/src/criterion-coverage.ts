...
export const CRITERION_COVERAGE: Record<string, CriterionCoverage> = {
  ...
  "3.3.3": {
    ...
    note:
      "Error Suggestion. THE CORPUS AND SIGNAL EXIST as of 2026-09-01 (16 case families, 34 cases, "
      + "`error-remedy-missing`); what is missing is a trained head, so this stays `reachable` until one "
      + "ships. Marking it assessed first is what `criterion-coverage.test.ts` refuses, correctly: the "
      + "claimed set must equal what the judge can actually return a finding for, or this map lies to a "
      + "consumer. It cost no recapture, which is why it was the tractable one of the three reachable "
      + "screen-reader criteria -- the form probe already submits and re-reads, so only a decider was "
      + "missing. The pair is deliberately SINGLE-CRITERION: both variants announce the error correctly "
      + "(aria-invalid, role=alert, focus moved) so both satisfy 3.3.1, and the only difference is whether "
      + "the message names a REMEDY. Measured on the first captured pair: good says 'invalid entry, Enter "
      + "the visit date as DD slash MM slash YYYY', bad says 'invalid entry, Invalid entry.' The remedies "
      + "vary in form (format, example, range, instruction, constraint) so a head cannot separate on one "
      + "phrase, and the signal matches an INSTRUCTION rather than a sentiment.",
  },
