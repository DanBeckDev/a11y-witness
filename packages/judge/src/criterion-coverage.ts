...
export const CRITERION_COVERAGE: Record<string, CriterionCoverage> = {
  ...
  "2.4.1": { status: "reachable", needs: ["screen-reader"], channels: ["focusOrder", "links"], note: "SCOPED 2026-08-22, and the earlier note here pointed at a rule that would OVER-CLAIM. It said 'a skip link is the first focusable element and announces as one', i.e. detect its presence — but W3C's Understanding page is explicit that a skip link is NOT required: headings alone are sufficient (H69), ARIA landmarks alone are sufficient (ARIA11), and 'it is not the intent of this success criterion to require authors to provide methods that are redundant to functionality provided by the user agent'. So a page with no skip link and ordinary headings PASSES, and every corpus page has an h1. **The presence half belongs to the static layer, not here**: whether a heading, landmark or skip link exists is a DOM fact axe-core answers exactly, while our own landmark sweep is documented as nondeterministic and is deliberately excluded from model evidence for that reason — the same call already made for 3.1.1, where 'the attribute is the fact'. What IS uniquely ours is a bypass mechanism that EXISTS AND DOES NOT WORK: a skip link whose target is missing or unfocusable, so activating it leaves the user exactly where they were. A static checker sees a link and a plausible href and passes it; only driving the thing shows that focus never moved. That is the same shape as 2.4.2's transition case and is the only 2.4.1 claim worth building." },
