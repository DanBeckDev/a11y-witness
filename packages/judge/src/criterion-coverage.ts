...
export const CRITERION_COVERAGE: Record<string, CriterionCoverage> = {
  ...
  "2.1.2": { status: "partial", needs: ["screen-reader"], channels: ["focusOrder"], note: "Keyboard trap, from `focusOrder`. ONE failure mode is assessed: focus STALLING — Tab pressed repeatedly and the same control announced each time, on a page whose sweep found controls it never reached. Unambiguous, because a control that will not release focus on Tab offers no documented means to leave. `decidedBy: \"rules\"`, and the one 2.1.2 in the real-page baseline (scotcourts) is this. NOT assessed: focus CONFINED to a group it cycles within. Two rules for that were built and withdrawn on 2026-08-28 after `rules-real-pages` measured 7 and 9 new findings on 86 conformant real pages — a consent banner or a date-picker overlay confines Tab BY DESIGN, and under 2.1.2 that conforms whenever the user can leave by a documented means. Measured: tfl.gov.uk ring 5 against 28 swept controls, three of them 'Accept all cookies'; the corpus case it was built for, ring 3 against 5. The same evidence. A THIRD attempt pressed Escape on the confinement and was inert: `anchorToTop` presses Escape as its first action before the walk, so a dialog that responds to it is already closed and a confined ring ALREADY means confined-after-Escape — the withdrawn rule's exact condition. Separating them requires ACTIVATING the dialog's dismiss control, which is a keyboard means of leaving and which `probeForms` deliberately withholds on pages we do not own. Blocked on that policy, not on a probe; until it is decided, a confined ring is not reported at all." },
