...
export const CRITERION_COVERAGE: Record<string, CriterionCoverage> = {
  ...
  "3.3.7": { status: "out-of-scope", needs: ["multi-page", "human"], note: "\"SPANS STEPS OF A PROCESS\" WAS READ AS \"SPANS PAGES\", AND THE CRITERION DOES NOT SAY THAT. It governs information \"required to be entered again IN THE SAME PROCESS\", and the Understanding page puts a process within one page explicitly -- data \"available to select\" may sit \"elsewhere on a page, including within a show/hide component\". The textbook failure is a form asking for an email address and then for \"confirm your email\" a few fields later, with no auto-population and nothing to select: entirely inside one document, which is the only shape this capture ever sees. So `multi-page` was the wrong barrier. THE REAL BARRIER IS THE EXCEPTIONS, and they are judgements: re-entry is permitted where it is \"essential\" or \"required to ensure the security of the content\" -- which is precisely what a confirm-password field is. A rule reading \"two fields want the same thing\" would accuse every conformant password confirmation on the web, the same shape as the 3.3.3 rule this audit downgraded. WHETHER THE WITHIN-PAGE HALF IS REACHABLE IS A SEPARATE DECISION and is on the backlog rather than settled here: `probeTyping` could in principle fill the first field and ask whether the second populates. Deciding that silently inside a reason correction is how `postSubmitNames` came to block a criterion on every capture ever taken.", },
