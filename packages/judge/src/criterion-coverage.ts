...
export const CRITERION_COVERAGE: Record<string, CriterionCoverage> = {
  ...
  // PARTIAL, NOT ASSESSED -- #251. This read `assessed` while the note four hundred characters later
  // said ONE of the criterion's FOUR categories is covered. Either the status was wrong or the note was,
  // and a reader could not tell which: `status` is what the report prints and what the coverage count
  // sums, and the count is a public number that reaches the board document and a stranger's report.
  //
  // 2.4.2 four entries up is `partial` for strictly less than this -- "PARTIAL, and the part is chosen
  // rather than incidental" -- so the file already had the right precedent and this entry did not follow
  // it. #34 says two criteria overstate their coverage; this is one of the two, in the file's own words.
  //
  // `needs: ["screen-reader"]`, and the first version of this change left it off on the reasoning that
  // the gap is a CORPUS gap rather than a missing source. `criterion-coverage.test.ts` refused that, and
  // it was right: `needs` names WHAT WOULD CLOSE IT, not what cannot be got. Its own test says why --
  // "without this the map degrades into the same undifferentiated `untested` bucket it exists to
  // replace". The uncovered categories are waiting state and progress, and this entry's note already
  // says they are "reachable, since a live region saying 'Loading...' is as audible as any other" -- a
  // live region announcement IS NVDA's own output, which is exactly what `screen-reader` denotes. 2.1.2,
  // 2.4.1 and 2.4.2 all carry the same value for the same reachable-but-partial reason.
