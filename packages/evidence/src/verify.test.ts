...
test("#30: a GET submit that only adds a query string did NOT navigate to another document", () => {
  const capture = submitNav(
    "http://REDACTED-INTERNAL-ADDRESS:5050/focus-removed-on-receipt-order/bad",
    "http://REDACTED-INTERNAL-ADDRESS:5050/focus-removed-on-receipt-order/bad?first=&second=&third=",
  );
...
test("#30: a submit that reaches a DIFFERENT PATH still counts as navigating away", () => {
  ...
  const capture = submitNav(
    "http://REDACTED-INTERNAL-ADDRESS:5050/checkout/details",
    "http://REDACTED-INTERNAL-ADDRESS:5050/checkout/confirmation?ref=A1",
  );
...
test("#30: a submit that reaches another ORIGIN has left the document, however similar the path", () => {
  const capture = submitNav("http://REDACTED-INTERNAL-ADDRESS:5050/search", "https://www.bing.com/search?q=x");
  assert.equal(submitNavigatedTheDocument(capture), true,
...
test("#30: identical from and to is not a navigation, and the query check does not invent one", () => {
  const url = "http://REDACTED-INTERNAL-ADDRESS:5050/form/bad?first=";
